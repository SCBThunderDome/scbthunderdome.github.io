...
In-game year, not real-world. The league is `2026` today; next season is
`2027`, and so on.
...
`HEAD` at `throughWeek` = null/0/1/2/15: ranks, scores
...
The league passes clean today. Verified by corrupting a copy of `scbthunderdome/` in
...
```
node tools/h2h.js                  every coach
node tools/h2h.js --coach Projekt  one coach, every meeting
node tools/h2h.js --summary        one line per coach
node tools/h2h.js --check          assertions; exits non-zero on failure
```
...
`team-colors.js` (repo root, shared by every league) holds one primary hex
...
`postseason-data.js` now exists (empty) in the league folder and is loaded by
