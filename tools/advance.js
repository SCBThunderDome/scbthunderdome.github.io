...
/* ============================================================
   ...
   USAGE
     node tools/advance.js --week 5 --next "2026-07-26 23:00"
     node tools/advance.js --week 2 --no-post
   ...
   FLAGS
     ...
     --next "..."      the next advance deadline, as a DATE, not a
                       sentence. "2026-07-26 23:00", or "2026-07-26"
                       for a deadline with no time shown. Eastern
                       either way — see /deadline.js. Carried over
   ...
   WEBHOOK — per league
     Read from env DISCORD_WEBHOOK_URL, or from tools/config.json:
       { "leagues": { "scbthunderdome": { "webhookUrl": "https://discord.com/..." } } }
...
function makeMentioner(cfg) {
  ...
  /* Case-insensitive index. config.json and league-data.js are edited
     by hand and independently, so a name will eventually be spelled
     with different capitalization in the two files. An exact-match
     lookup would then silently drop the ping and just bold the name.
     Lowercasing both sides fixes that whole class of bug. No two
     distinct coaches collide when lowered. */
  const byLower = {};
...
/* Rewrite ONLY the deadline, leaving currentWeek and statusLine
   ...
   Takes the same date-shaped value updateSeason() does, and writes
   the same PAIR — timestamp plus generated sentence. A deadline
   changed here and a deadline changed by an advance have to end up
   in identical shape, or the heads-up would work after one and not
   the other. "" clears both, which is how the badge gets hidden. */
function updateNextAdvance(leagueFile, nextAdvanceAt) {
  const src = fs.readFileSync(leagueFile, "utf8");
  const { open, close, body } = seasonBlock(src, path.basename(leagueFile));

  const stored = Deadline.canonical(nextAdvanceAt);
  if (stored === null) {
    die(`refusing to write an unreadable deadline: ${JSON.stringify(nextAdvanceAt)}`);
  }
  const text = stored ? Deadline.formatDeadline(stored) : "";
...
function webhookUrl(cfg, slug) {
  ...
  if (slug === "scbthunderdome" && cfg.webhookUrl && !cfg.webhookUrl.includes("PASTE")) {
  ...
  if (!Deadline.parseAt(value)) {
    die(
      `--next "${value}" isn't a date this can work with.\n` +
        ...
        '    --next "2026-07-26 23:00"   Sunday, July 26th — 11:00 PM EDT\n' +
...
async function main() {
  ...
  const L = resolveLeague(args.league || "scbthunderdome");
  const slug = L.slug;
  const meta = { label: L.label, dir: L.dir };
  const paths = L.paths;
  const siteUrl = L.siteUrl;

  const week = parseWeek(args.week, '--week 5 --next "2026-07-26 23:00"');
