...
const SITE_ROOT = "https://scbthunderdome.github.io";
...
/*
   ...
   before that week's poll has been transcribed. See top25GateError()
   below. */
const LEAGUES = {
  scbthunderdome: {
    label: "SCB Thunderdome",
    dir: "scbthunderdome",
    /* false at launch: no Top 25 has been transcribed yet, and a gate
       with no poll behind it would block every advance. Flip to true
       once top25-data.js is being kept current. */
    gateOnTop25: false,
  },
};
...
/* ------------------------------------------------------------
   PATHS
   ------------------------------------------------------------ */
function resolveLeague(slug = "scbthunderdome") {
  if (!LEAGUES[slug]) {
...
/* The week the in-game poll becomes the CFP Top 25 and the bracket
   ...
   moves it, they all move together. */
...
function top25GateError(data, week, league) {
  ...
  if (Number(week) > REGULAR_FINAL_WEEK) {
    ...
    if (!settled.length) {
      ...
      return (
        ...
        `    node tools/cfp.js --week ${REGULAR_FINAL_WEEK} --bracket bracket.txt --final`
      );
    }
    return null;
  }
  ...
  if (Number(week) >= CFP_ERA_WEEK) {
    ...
    const fix = missing.includes("CFP Top 25")
      ? `    node tools/cfp.js --week ${week} --poll poll.txt --bracket bracket.txt`
      : `    node tools/cfp.js --week ${week} --bracket bracket.txt`;
...
/* ------------------------------------------------------------
   ...
   for every dynasty, because a vacation is a fact about a
