...
function testFileSurgery() {
  console.log("\n  Rewriting the live files (temp copies of the real Thunderdome data)\n");
  ...
  const copy = (name) => {
    ...
    fs.copyFileSync(path.join(ROOT, "scbthunderdome", name), to);
    return to;
  };
  ...
  ok(
    ...
    (reset.text.match(/name:\s*"/g) || []).length ===
      (read("scbthunderdome/league-data.js").match(/name:\s*"/g) || []).length
  );
...
function testValidate() {
  ...
  const base = { action: "rollover", league: "scbthunderdome", year: 2026, confirm: true, actor: "Tester" };
...
async function testPanel(JSDOM, VirtualConsole) {
  ...
  console.log("\n  In the offseason hold, season finished cleanly\n");
  {
    ...
    ok("it names the archive folder", /scbthunderdome\/seasons\/2026\//.test($("rollover-hint").textContent));
