/* ============================================================
   ...
   --------
     POST /whoami   { code }
       -> { name: "Dave", leagues: ["scbthunderdome"] }
...
/* The once-a-year action: archive a finished season and start the
   ...
   answers even while they hold the same league. */
...
/* ------------------------------------------------------------
   ...
   ACCESS_CODES is a secret holding JSON:
     ...
     {
       "long-random-string": { "name": "Dave",  "leagues": ["scbthunderdome"] },
       "another-long-one":   { "name": "Marcus","leagues": ["scbthunderdome"] }
     }
...
function checkPayload(payload, who) {
  ...
  /* THE DEADLINE IS A DATE, NOT A SENTENCE — the sentence the site
     ...
     round-trip through Actions for an obviously wrong value.
       ...
       2026-08-13T23:00:00-04:00   a day and a time
       2026-08-13                  a day, no time shown
       ""                          cleared, which hides the badge

     Both actions are checked the same way on purpose: a deadline set
     by an advance and one set by the deadline editor have to end up
     in identical shape, or the advance-day heads-up would work after
     one and not the other. */
  if (isSeasonEdit && payload.nextAt !== undefined) {
    ...
    if (!ok) return "deadline isn't a date like 2026-08-13T23:00:00-04:00";
  }

  if (action === "deadline" && payload.nextAt === undefined) {
    return "missing deadline date";
  }
...
async function dispatchEvent(env, eventType, clientPayload) {
  ...
  const res = await fetch(`https://api.github.com/repos/${repo}/dispatches`, {
    ...
    headers: {
      ...
      "User-Agent": "scb-thunderdome-admin",
